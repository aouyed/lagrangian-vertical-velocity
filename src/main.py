...
def flow(ds_s):
    ...
    dates=ds_s['time'].values
    print(dates)
    ds_unit=ds_s.loc[{'time':dates[0]}]
    ds_unit['cloud_top_height'].plot.imshow(vmin=0, vmax=10)
    plt.savefig('cloud_top_height_0.png')
  
    frame0=ds_unit[flow_var].values
    ...
    for date in dates:
        print(date)
        ds_unit=ds_s.loc[{'time':date}]
        frame=ds_unit[flow_var].values
        mask=np.isnan(frame)
        frame=np.nan_to_num(frame)
        ...
        dzdt=1000/1800*dz
        pzpt=1000/1800*pz
        ds_unit['flow_x']=(('image_y','image_x'),flowd[:,:,0])
